...
<?php
	$txnQ = $db->query("SELECT * FROM cart WHERE paid = 1 ORDER BY id DESC LIMIT 5");
	$results = array();
	while ($row = mysqli_fetch_assoc($txnQ)) {
		$results[] = $row;	
	}
	$row_count = $txnQ->num_rows;
	$used_ids = array();
...
<div id="recent_widget">
	<table class="table table-condensed">
		...
		<tr>
			...
			<td>
				<a class="text-primary" href="#" onclick="detailsmodal(<?=$id;?>)">View</a>
			</td>
		</tr>
		<?php endforeach; ?>
	</table>
